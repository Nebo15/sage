defmodule Sage.ExecutorTest do
  ...
  describe "tracers" do
    test "modules are notified for all operations" do
      result =
        new()
        |> run(:step1, transaction(:t1), compensation())
        |> run(:step2, transaction(:t2), compensation())
    ...
    test "funcions are notified for all operations" do
      result =
        ...
        |> run_async(:step4, transaction(:t4), not_strict_compensation())
        |> run(:step5, transaction_with_error(:t5), compensation())
        |> with_tracer(&Sage.TestTracer.handle_event/3)
        |> execute(test_pid: self())
    ...
    test "mfas are notified for all operations" do
      result =
        new()
        |> run(:step1, transaction(:t1), compensation())
        |> run(:step2, transaction(:t2), compensation())
        ...
        |> run_async(:step4, transaction(:t4), not_strict_compensation())
        |> run(:step5, transaction_with_error(:t5), compensation())
        |> with_tracer({Sage.TestTracer, :handle_event, []})
        |> execute(test_pid: self())
